...
class EvolutionCoordinator:
        ...
        while True:
            try:
                ...
                # Рендеринг хромосомы.
                    ...
                    chromosome, pins, thread_scale = data
                    size = render_params.get('size', 500)
                    cm = CanvasManager(size, pins)
                    visual_thickness = max(1, int(1 * thread_scale))
    ...
    def _prepare_and_run_evolution(self):
        ...
        try:
            ...
            def greedy_callback(status: str, path: List[int]):
                ...
                render_data = (path, config['pins'], self.state.vars['thread_scale'].get())
    ...
    def _handle_done(self, data: str):
        self.ui_manager.widgets['status_label'].config(text=data)
        self.ui_manager.toggle_ui_state(False)
        if self.manager_thread: self.manager_thread.join()
    ...
    def request_canvas_update(self, key: Any, data: Any, render_params: Dict[str, Any] = {}):
        ...
        if not canvas or (canvas.winfo_width() < 10): return
    ...
    def _update_progress_stats(self):
        ...
        if not self.state.is_running or not self.manager_thread or not self.manager_thread.is_alive(): return
    ...
    def create_difference_map_overlay(self, diff_map: np.ndarray, base_image: np.ndarray) -> np.ndarray:
        """
        Создает информативную карту расхождений, накладывая цвета на
        полупрозрачное оригинальное изображение.
        """
        # Нормализуем карту расхождений от -1 до 1.
        max_abs_val = np.max(np.abs(diff_map))
        if max_abs_val == 0: max_abs_val = 1.0
        norm_diff = diff_map / max_abs_val

        # Создаем цветную маску (красный/синий).
        h, w = diff_map.shape
        color_mask = np.zeros((h, w, 3), dtype=np.uint8)
        
        # Красный: ниток СЛИШКОМ МНОГО (rendered < target, diff > 0)
        red_mask = norm_diff > 0.1
        color_mask[red_mask] = [255, 0, 0]

        # Синий: ниток СЛИШКОМ МАЛО (rendered > target, diff < 0)
        blue_mask = norm_diff < -0.1
        color_mask[blue_mask] = [0, 0, 255]

        # Конвертируем основное изображение в PIL для наложения.
        base_pil = Image.fromarray(base_image).convert("RGBA")
        color_pil = Image.fromarray(color_mask).convert("RGBA")

        # Создаем альфа-канал для цветной маски, чтобы она была полупрозрачной.
        alpha = Image.fromarray((np.abs(norm_diff) * 150).astype(np.uint8))
    ...
    def prune_chromosome(self):
        ...
        if original_len != pruned_len:
            ...
            render_data = (pruned_chromo, self.state.get_config()['pins'], self.state.vars['thread_scale'].get())
            self.request_canvas_update('overall', render_data, {'size': 500})
    ...
    def export_svg(self):
        ...
        with open(filepath, 'w') as f:
            ...
            f.write(f'  <path d="{path_data}" stroke="black" stroke-width="0.5" fill="none" />\n')
    ...
    def save_progress(self):
        """Сохраняет прогресс (лучшую хромосому и настройки)."""
        if not self.state.overall_best_chromosome: return
        filepath = filedialog.asksaveasfilename(defaultextension=".pkl", filetypes=[("Progress files", "*.pkl")])
        if not filepath: return
        ...
        messagebox.showinfo("Сохранение", "Прогресс успешно сохранен.")

    def load_progress(self):
        """Загружает прогресс."""
        filepath = filedialog.askopenfilename(filetypes=[("Progress files", "*.pkl")])
        if not filepath: return
        ...
        try:
            ...
            self.state.original_image = Image.open(loaded_data['image_path'])
            self.state.original_image_path = loaded_data['image_path']
            self.root.redraw_target_canvas()
            
            # Применяем настройки
            for key, value in loaded_data['config'].items():
                if key in self.state.vars:
                    self.state.vars[key].set(value)
            
            # Устанавливаем хромосому как лучший результат
            self.state.overall_best_chromosome = loaded_data['chromosome']
            self.best_chromosomes_by_key['overall'] = loaded_data['chromosome']
            
            # Обновляем UI
            self.ui_manager.toggle_ui_state(False) # Разблокируем кнопки экспорта
            self.ui_manager.setup_for_new_run(self.state.get_config()['num_islands'])
            self.ui_manager.update_title()
            
            render_data = (self.state.overall_best_chromosome, self.state.get_config()['pins'], self.state.vars['thread_scale'].get())
            self.request_canvas_update('overall', render_data, {'size': 500})
            
            messagebox.showinfo("Загрузка", "Прогресс успешно загружен.")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось загрузить файл прогресса: {e}")
